...
func (r *ProductRepositoryImpl) Update(ctx context.Context, product models.Product) error {
	_, err := r.db.Exec(ctx, "UPDATE products SET name=$1, description=$2, attributes=$3, weight=$4, barcode=$5 WHERE id=$6",
		product.Name, product.Description, product.Attributes, product.Weight, product.Barcode, product.ID)
	return err
}
